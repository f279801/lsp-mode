...
(declare-function helm-build-sync-source "ext:helm-source" t)
...
(defun lsp-yaml--get-supported-schemas ()
  ...
  (progn
    (if lsp-yaml-schema-store-enable
        (if (not lsp-yaml--schema-store-schemas-alist)
            (progn
...
(defun lsp-yaml--helm-schema-candidates ()
  "Build helm source for schema candidates."
  (cl-loop for schema in (lsp-yaml--get-supported-schemas)
	   collect (cons
...
(defun lsp-yaml-set-buffer-schema (uri)
  ...
  (let* ((workspace-path (file-relative-name
    ...
    (lsp-register-custom-settings
     '(("yaml.schemas" lsp-yaml-schemas t)))
...
(defun lsp-yaml-select-buffer-schema ()
  ...
  (let* ((schema (helm :sources
                       (helm-build-sync-source "Select buffer schema"
                         :candidates #'lsp-yaml--helm-schema-candidates
                         :multiline 100)
                       :buffer "*lsp-yaml supported schemas*"))
